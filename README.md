...
- `settings.{view,edit}`
- `backups.{view,download,create,delete}`
- `queue.{view,manage}` (`manage` — retry and delete failed jobs)
...
On an existing database the `2026_09_25_000003` migration adds `users.export`,
`users.impersonate` and `backups.download`: roles that could export or download before
keep that ability, and impersonation goes to the superadmin role only.
...
`/robots.txt` and `/sitemap.xml` are generated by `SeoController` from the same settings.
With `indexable` off, robots.txt closes the whole site; with it on, only `/admin` is closed,
and the `sitemap` flag publishes `/sitemap.xml` and announces it in robots.txt. The template
has no public pages, so the sitemap starts empty: register your URLs in a service provider
through `App\Support\Sitemap` (see the example in its docblock). Admin responses always carry
...
- **Notifications** — other users' actions from the past week in the bell; the badge counts
  unread items (refreshed every minute and when the tab regains focus) and opening the
  bell marks them read. Category chips filter the list, the "Настроить" panel mutes whole
  categories per user, and a burst of failed sign-ins for one email shows as one item with
  a `×N` counter.
- **Session expiry** — a minute before an idle session ends, a modal offers to stay signed
  in; while the user is active the session is kept alive even on one page. Background polling
  pauses when idle, and any request that hits an expired session (401/419) shows a toast and
  leads back to the login page instead of failing silently.
- **Accounts** — blocking (`is_active`), a forced password change on next login, last-login
  time, and a self-service profile with active sessions (`/admin/profile`). Changing the
  email asks for the current password.
- **Users list** — status and "must change password" filters, sorting by last login,
  bulk block/unblock with an optional reason (shown on the user page), CSV export of the
  current selection, a column picker remembered in the browser, a password generator in the
  form and the user's recent activity on their page. "Back to list" links on the user and
  role pages return to the list with its filters and page.
- **Sign in as** — a holder of `users.impersonate` can open the panel as another active,
  non-superadmin user they are allowed to manage; a banner offers the way back. Log entries
  made meanwhile keep the real user as the impersonator.
- **Activity log** — filters by action, object type, user and dates, CSV export, links to
  the affected records, a per-record history (`subject_type` + `subject_id`); logins,
  logouts, ended sessions, settings changes (with a diff) and impersonation are recorded.
  A filter shows actions made while signed in as someone else, and the diff highlights
  the changed part of long values and lists.
- **Media library** — server-side type filter, search, sorting and page size, file details,
  copying the link, replacing the file behind an existing record, alt text and folders
  (with a bulk "move to folder"), upload checks before sending, failed-upload reports,
  and a warning before deleting a file used as the favicon or OG image. JPEG orientation
  from EXIF is applied during conversion. Also: cropping with aspect presets, a focal point
  that keeps the subject in view in cropped previews, skipping duplicate uploads (same
  content), a "used / unused" filter, renaming and clearing folders, dragging files onto a
  folder, and responsive copies served through `srcset` (WebP, or AVIF with
  `MEDIA_IMAGE_FORMAT=avif` when GD supports it).
- **Backups** — list, download, delete and on-demand creation of `app:db-backup` dumps
  (queued, with a pending state and the last failure shown on the page);
  see [Backups](#backups).
- **Job queue** — pending jobs, the oldest waiting job and failed jobs with retry/delete.
- **Dashboard** — blocked users, sign-ins and failed sign-ins over 24 hours, media
  storage size, and queue/backup health for those with the matching permissions.
...
Media library files go to the disk set in `MEDIA_DISK` (default `public`, needs
`storage:link`). For S3 or a compatible store, set `MEDIA_DISK=s3`, fill in the `AWS_*`
variables (`AWS_URL` for a CDN or public bucket URL, `AWS_ENDPOINT` for MinIO) and run
`composer require league/flysystem-aws-s3-v3`. Files must be publicly readable. The
production CSP allows the disk's origin automatically. Changing the disk does not move
files that were already uploaded.

### Backups

`app:db-backup` writes `db-YYYYmmdd-HHiiss[-tag].sqlite|sql[.enc]` (UTC) to `BACKUP_PATH`
(default `storage/app/backups`). SQLite is dumped with `VACUUM INTO`, MySQL/PostgreSQL
with their dump tools. Scheduled and manual (`--tag=manual`, the UI button) dumps are
rotated separately: `BACKUP_KEEP` (7) and `BACKUP_KEEP_MANUAL` (5).

- **Dump tools** — MySQL/MariaDB uses `mariadb-dump` when available, otherwise `mysqldump`;
  PostgreSQL uses `pg_dump --clean --if-exists`. The Docker image ships the MariaDB and
  PostgreSQL clients; a manual install needs them on the `PATH`.
- **Verification** — every dump is checked before it is kept: SQLite with
  `PRAGMA integrity_check`, SQL dumps by the tool's completion marker, encrypted files by
  decrypting them. A failed check deletes the file and reports an error.
- **The UI button** queues a `CreateBackup` job, so a worker must be running (or
  `QUEUE_CONNECTION=sync`, where the result is reported right away). A second dump is refused
  while one is pending.
- **Restore** — `php artisan app:db-restore <file> --force` restores SQLite, MySQL and
  PostgreSQL dumps (encrypted ones too) after taking a `prerestore` safety dump
  (`--no-safety-backup` skips it). Stop the queue worker and scheduler first.
- **Encryption** — set `BACKUP_ENCRYPTION_KEY` (generate one with `php artisan app:backup-key`)
  and dumps are encrypted with libsodium (the `sodium` PHP extension); decrypt a file with
  `php artisan app:db-backup-decrypt <file> --output=<path>`. Keep the key outside the server.
- **Offsite copy** — set `BACKUP_DISK` (a filesystem disk) and `BACKUP_DISK_PATH`; every dump
  is uploaded there with the same rotation. Deleting a dump in the UI removes only the local file.
...
- **A large upload is processed twice or lands in `failed_jobs` as "attempted too many times"** —
  the queue's `retry_after` (`DB_QUEUE_RETRY_AFTER`/`REDIS_QUEUE_RETRY_AFTER`, default 330 s) must stay
  above the longest job timeout (`UploadMedia`, 300 s); otherwise another worker picks the job up
